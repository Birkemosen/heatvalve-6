...
- **config_dev.yaml** bruger relative stier (`boards/`, `core/`, `zones/`) og `components` (ikke `heatvalve-6/components`), så alt kører fra repo-roden.
- **Device-navn** er `heatvalve-6-dev`, så OTA overskriver ikke en evt. produktions-enhed med navnet `heatvalve-6`.
- Samme indhold som `config.yaml`; kun stier og device-navn er ændret.
