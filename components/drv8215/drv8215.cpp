...
bool Drv8215::init() {
  ...
  // CONFIG4: PMODE=1, I2C_BC=1, EN=0, PH=0 → coast on entry
  ...
  write_reg(REG_CONFIG3, CONFIG3_DEFAULT);

  // CONFIG0: enable hardware stall detection
  write_reg(REG_CONFIG0, CONFIG0_EN_STALL);

  // RC_CTRL0: current-sense gain 5320 µA/A (covers 10–60 mA range via 5.1 kΩ RIPROPI)
  write_reg(REG_RC_CTRL0, RC_CTRL0_GAIN_5320);
...
void Drv8215::clear_fault() {
  write_reg(REG_CONFIG0, CONFIG0_CLR_FLT);  // CLR_FLT=1
  write_reg(REG_CONFIG0, CONFIG0_EN_STALL); // CLR_FLT=0
}
