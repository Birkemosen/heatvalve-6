...
`,Mr=h({tag:"connectivity-card",render:qt,onMount(e,t){let r=t.querySelector(".cc-ip"),o=t.querySelector(".cc-ssid"),n=t.querySelector(".cc-mac"),a=t.querySelector(".cc-up");function l(){r.textContent=S(i.ip)||"---",o.textContent=S(i.ssid)||"---",n.textContent=S(i.mac)||"---",a.textContent=ue(b(i.uptime))}p(i.ip,l),p(i.ssid,l),p(i.mac,l),p(i.uptime,l),l()}});var Pt=`
.graph-widgets {
  display: grid;
  gap: 12px;
}
...
`;z("zone-grid",Wt);var jt=()=>'<div class="zone-grid"></div>',Pr=h({tag:"zone-grid",render:jt,onMount(e,t){for(let r=1;r<=6;r++)t.appendChild(M("zone-card",{zone:r}))}});var Vt=`
.zone-card {
	display: grid;
...
`;z("zone-sensor-card",Xt);var Yt=()=>{let e="<option>None</option>";for(let t=1;t<=8;t++)e+="<option>Probe "+t+"</option>";return`
  ...
  `};function Jt(e,t){let r=e.value,o="<option>None</option>";for(let n=1;n<=6;n++)n!==t&&(o+="<option>Zone "+n+"</option>");e.innerHTML=o,e.value=r||"None"}var sa=h({tag:"zone-sensor-card",render:Yt,onMount(e,t){let r=t.querySelector(".zs-probe"),o=t.querySelector(".zs-source"),n=t.querySelector(".zs-zigbee"),a=t.querySelector(".zs-ble"),l=t.querySelector(".zs-sync"),d=t.querySelector(".zs-row-zigbee"),g=t.querySelector(".zs-row-ble"),m=0;function u(){return k("selectedZone")}function f(){let x=u();m!==x&&(Jt(l,x),m=x);let w=S(s.probe(x)),E=S(s.tempSource(x))||"Local Probe",R=S(s.syncTo(x))||"None",J=S(s.zigbee(x))||"",ne=S(s.ble(x))||"";w&&(r.value=w),o.value=E,l.value=R,document.activeElement!==n&&(n.value=J),document.activeElement!==a&&(a.value=ne),d.style.display=E==="Zigbee MQTT"?"":"none",g.style.display=E==="BLE Sensor"?"":"none"}function _(x){let w=u();(x===s.probe(w)||x===s.tempSource(w)||x===s.syncTo(w)||x===s.zigbee(w)||x===s.ble(w))&&f()}r.addEventListener("change",()=>{U(u(),"zone_probe",r.value)}),o.addEventListener("change",()=>{U(u(),"zone_temp_source",o.value)}),l.addEventListener("change",()=>{U(u(),"zone_sync_to",l.value)}),n.addEventListener("change",()=>{te(u(),"zone_zigbee_device",n.value)}),a.addEventListener("change",()=>{te(u(),"zone_ble_mac",a.value)}),F("selectedZone",f);for(let x=1;x<=6;x++)p(s.probe(x),_),p(s.tempSource(x),_),p(s.syncTo(x),_),p(s.zigbee(x),_),p(s.ble(x),_);f()}});var Kt=`
.zone-room-card {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 18px;
...
`,ma=h({tag:"zone-room-card",render:Qt,onMount(e,t){let r=t.querySelector(".zr-friendly"),o=t.querySelector(".zr-area"),n=t.querySelector(".zr-spacing"),a=t.querySelector(".zr-pipe"),l=t.querySelectorAll(".wall-btn");function d(){return k("selectedZone")}function g(){let u=d();document.activeElement!==r&&(r.value=Z(u)||""),document.activeElement!==o&&(o.value=b(s.area(u))!=null?String(b(s.area(u))):""),document.activeElement!==n&&(n.value=b(s.spacing(u))!=null?String(b(s.spacing(u))):""),a.value=S(s.pipeType(u))||"Unknown";let f=S(s.exteriorWalls(u))||"None",_=f==="None"?[]:f.split(",").filter(Boolean);l.forEach(x=>{let w=x.dataset.wall;x.classList.toggle("active",w==="None"?_.length===0:_.includes(w))})}function m(u){let f=d();(u===s.area(f)||u===s.spacing(f)||u===s.pipeType(f)||u===s.exteriorWalls(f))&&g()}r.addEventListener("change",()=>{Re(d(),r.value)}),o.addEventListener("change",()=>{_e(d(),"zone_area_m2",o.value)}),n.addEventListener("change",()=>{_e(d(),"zone_pipe_spacing_mm",n.value||"200")}),a.addEventListener("change",()=>{U(d(),"zone_pipe_type",a.value)}),l.forEach(u=>{u.addEventListener("click",()=>{let f=u.dataset.wall,_=S(s.exteriorWalls(d()))||"None",x=_==="None"?[]:_.split(",").filter(Boolean);if(f==="None")x=[];else{let E=x.indexOf(f);E>=0?x.splice(E,1):x.push(f)}let w=["N","S","E","W"].filter(E=>x.includes(E));te(d(),"zone_exterior_walls",w.length?w.join(","):"None")})}),F("selectedZone",g),F("zoneNames",g);for(let u=1;u<=6;u++)p(s.area(u),m),p(s.spacing(u),m),p(s.pipeType(u),m),p(s.exteriorWalls(u),m);g()}});var eo=`
.flow-wrap {
  width: 100%;
  border-radius: 18px;
  overflow: hidden;
...
`,Sa=h({tag:"diag-i2c",render:wo,onMount(e,t){let r=t.querySelector("#i2c-result");function o(){r.textContent=k("i2cResult")||"No scan has been run yet."}t.querySelector("#btn-i2c-scan").addEventListener("click",()=>{Ge()}),F("i2cResult",o),o()}});var _o=`
.diag-zone {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 18px;
...
`;z("diag-zone",_o);function So(e){return e==="HEATING"?"s-heating":e==="IDLE"?"s-idle":e==="FAULT"?"s-fault":""}function it(e){return e!=null?Number(e).toFixed(2)+"x":"---"}function lt(e){return e!=null?Number(e).toFixed(0):"---"}function ko(e){return e!=null?Number(e).toFixed(2)+"C":"---"}var Eo=()=>{let e="";for(let t=1;t<=6;t++)e+=`
...
`;z("diag-zone-motor",Do);var Ao=e=>{let t=e.zone||k("selectedZone")||1,r="";for(let o=1;o<=6;o++)r+='<option value="'+o+'"'+(o===t?" selected":"")+">Zone "+o+"</option>";return`
  ...
  `},Ua=h({tag:"diag-zone-motor-card",render:Ao,onMount(e,t){let r=Number(e.zone||k("selectedZone")||1),o=!!k("manualMode"),n=t.querySelector(".manual-mode-toggle"),a=t.querySelector(".motor-gated"),l=t.querySelector(".motor-zone-select"),d=t.querySelector(".motor-target-input"),g=t.querySelector(".motor-open-btn"),m=t.querySelector(".motor-close-btn"),u=t.querySelector(".motor-stop-btn");function f(w){o=!!w,n&&(n.classList.toggle("on",o),n.setAttribute("aria-checked",o?"true":"false")),a&&a.classList.toggle("locked",!o),[l,d,g,m,u].forEach(E=>{E&&(E.disabled=!o)})}function _(){let w=!o;if(f(w),w){ke(!0);for(let E=1;E<=6;E++)Se(E)}else ke(!1)}function x(){let w=b(s.motorTarget(r));d&&w!=null?d.value=Number(w).toFixed(0):d&&(d.value="0")}l==null||l.addEventListener("change",()=>{r=Number(l.value||1),x()}),n==null||n.addEventListener("click",_),n==null||n.addEventListener("keydown",w=>{w.key!==" "&&w.key!=="Enter"||(w.preventDefault(),_())});for(let w=1;w<=6;w++)p(s.motorTarget(w),x);x(),f(o),F("manualMode",()=>{f(!!k("manualMode"))}),d==null||d.addEventListener("change",w=>{if(!o)return;let E=w.target.value;$e(r,E)}),g==null||g.addEventListener("click",()=>{o&&Ue(r,1e4)}),m==null||m.addEventListener("click",()=>{o&&Xe(r,1e4)}),u==null||u.addEventListener("click",()=>{o&&Se(r)})}});var Oo=`
.diag-zone-recovery {
  background: var(--panel-bg-vibrant);
  border: 1px solid var(--panel-border);
  border-radius: 18px;
...
`;z("diag-zone-recovery",Oo);var To=e=>{let t=e.zone||k("selectedZone")||1,r="";for(let o=1;o<=6;o++)r+='<option value="'+o+'"'+(o===t?" selected":"")+">Zone "+o+"</option>";return`
...
`;z("settings-manifold-card",Ro);var qo=()=>{let e="";for(let r=1;r<=8;r++)e+="<option>Probe "+r+"</option>";let t="";for(let r=1;r<=8;r++)t+='<div class="probe-cell"><div class="probe-name">Probe '+r+'</div><div class="probe-temp" data-probe="'+r+'">---</div></div>';return`
...
`;z("settings-control-card",Po);var Zo=()=>`
...
`;z("settings-motor-calibration-card",Io);var Y=[{cls:"safe-runtime",key:"generic_runtime_limit_seconds",id:i.genericRuntimeLimitSeconds,label:"Max Safe Runtime",unit:"s"},{cls:"close-threshold",key:"close_threshold_multiplier",id:i.closeThresholdMultiplier,label:"Close Endstop Threshold",unit:"x"},{cls:"close-slope-threshold",key:"close_slope_threshold",id:i.closeSlopeThreshold,label:"Close Endstop Slope",unit:"mA/s"},{cls:"close-slope-floor",key:"close_slope_current_factor",id:i.closeSlopeCurrentFactor,label:"Close Endstop Slope Floor",unit:"x"},{cls:"open-threshold",key:"open_threshold_multiplier",id:i.openThresholdMultiplier,label:"Open Endstop Threshold",unit:"x"},{cls:"open-slope-threshold",key:"open_slope_threshold",id:i.openSlopeThreshold,label:"Open Endstop Slope",unit:"mA/s"},{cls:"open-slope-floor",key:"open_slope_current_factor",id:i.openSlopeCurrentFactor,label:"Open Endstop Slope Floor",unit:"x"},{cls:"open-ripple-limit",key:"open_ripple_limit_factor",id:i.openRippleLimitFactor,label:"Open Ripple Limit",unit:"x"},{cls:"relearn-movements",key:"relearn_after_movements",id:i.relearnAfterMovements,label:"Relearn After Movements",unit:"count"},{cls:"relearn-hours",key:"relearn_after_hours",id:i.relearnAfterHours,label:"Relearn After Hours",unit:"h"},{cls:"learn-min-samples",key:"learned_factor_min_samples",id:i.learnedFactorMinSamples,label:"Learned Factor Min Samples",unit:"count"},{cls:"learn-max-deviation",key:"learned_factor_max_deviation_pct",id:i.learnedFactorMaxDeviationPct,label:"Learned Factor Max Deviation",unit:"%"}],Ho=()=>{let e=`
