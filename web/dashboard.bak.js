...
(function () {
  ...
  function pushSelectedZone() {
    ...
    return postHv6API('/settings/number?key=selected_zone&value=' + selectedZone + '&zone=' + selectedZone);
  }
      ...
      '<main class="shell">' +
        '<header class="topbar">' +
          '<div class="topbar-head">' +
            '<div class="top-brand"><div class="side-brand">HEATVALVE-6</div></div>' +
          ...
          '</div>' +
        '</header>' +
        '<div class="reconn" id="reconn">CONNECTION LOST - RECONNECTING...</div>' +
...
// ==================================================
  ...
  function setLive(v) {
    ...
    if (rc) rc.className = v ? 'reconn' : 'reconn show';
  ...
  function postHv6API(path) {
    pendingWrites += 1;
    updateSyncState();
    return fetch(hv6ApiBase() + path, { method: 'POST' })
      .catch(function () {})
      .finally(function () {
  ...
  function postHv6Command(command, zone) {
    var path = '/commands?command=' + encodeURIComponent(command);
    if (typeof zone === 'number' && zone >= 1 && zone <= NZ) {
      path += '&zone=' + zone;
    }
    return postHv6API(path);
  }
...
// ==================================================
  ...
  window._hv6 = {
    ...
    adj: function (z, delta) {
      ...
      postHv6API('/zones/' + z + '/setpoint?setpoint_c=' + encodeURIComponent(t));
    },
    ...
    tZone: function (z) {
      ...
      postHv6API('/zones/' + z + '/enabled?enabled=' + (!on ? 'true' : 'false'));
    },
    ...
    tDrv: function () {
      ...
      postHv6API('/drivers/enabled?enabled=' + (!on ? 'true' : 'false'));
    },
    pickZone: function (z) {
      selectedZone = Math.max(1, Math.min(NZ, Number(z) || 1));
    ...
    pB: function (n) {
      if (n === 'i2c_scan') {
        ...
        postHv6Command('i2c_scan');
        return;
      }

      if (n === 'calibrate_all_motors') {
        postHv6Command('calibrate_all_motors');
        return;
      }

      var faultMatch = n.match(/^zone_(\d+)_reset_fault$/);
      if (faultMatch) {
        postHv6Command('motor_reset_fault', Number(faultMatch[1]));
        return;
      }

      var relearnMatch = n.match(/^zone_(\d+)_reset_relearn$/);
      if (relearnMatch) {
        postHv6Command('motor_reset_and_relearn', Number(relearnMatch[1]));
        return;
      }

      var factorsMatch = n.match(/^zone_(\d+)_reset_factors$/);
      if (factorsMatch) {
        postHv6Command('motor_reset_learned_factors', Number(factorsMatch[1]));
        return;
      }

      addActivity('Unsupported command: ' + n);
    },
    sS: function (n, v) {
      setEntity('select-' + n, { state: v });
      postHv6API('/settings/select?key=' + encodeURIComponent(n) + '&value=' + encodeURIComponent(v) + '&zone=' + selectedZone);
    },
    sN: function (n, v) {
      var num = Number(v);
      if (!isNaN(num)) setEntity('number-' + n, { value: num });
      postHv6API('/settings/number?key=' + encodeURIComponent(n) + '&value=' + encodeURIComponent(v) + '&zone=' + selectedZone);
    },
    sT: function (n, v) {
      setEntity('text-' + n, { state: v });
      postHv6API('/settings/text?key=' + encodeURIComponent(n) + '&value=' + encodeURIComponent(v) + '&zone=' + selectedZone);
    },
    ...
    tAutoLearn: function () {
      ...
      setEntity('switch-auto_apply_learned_factors', { value: !on });
      postHv6API('/motor/auto_learn?enabled=' + (!on ? 'true' : 'false'));
    },
    sSource: function (v) {
    ...
    tEW: function (z, dir) {
      ...
      if (dir === 'NONE') {
        ...
        postHv6API('/settings/text?key=zone_exterior_walls&value=' + encodeURIComponent('None') + '&zone=' + z);
        return;
      }
      var next = '';
      ...
      postHv6API('/settings/text?key=zone_exterior_walls&value=' + encodeURIComponent(next || 'None') + '&zone=' + z);
    },
    ...
    tManualMode: function () {
      ...
      updateManualModeUI();
      postHv6API('/manual_mode?enabled=' + (manualMode ? 'true' : 'false'));
    },
  };
