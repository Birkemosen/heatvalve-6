...
var FD_COL_FLOW_X = FD_EX + 300;
var FD_COL_RET_X = FD_EX + 372;
var FD_COLOR_DISABLED = '#6E7E96';
var FD_COLOR_EMPTY = '#5C6B85';
var FD_COLOR_FLOW_LOW = '#8a508f';
var FD_COLOR_FLOW_MID = '#BC5090';
var FD_COLOR_FLOW_HIGH = '#FF8531';
var FD_COLOR_FLOW_HOT = '#FFA600';
var FD_COLOR_RETURN = '#8a508f';
var FD_COLOR_ZONE_ON = '#FFEAD2';
var FD_COLOR_ZONE_OFF = '#6E7E96';
var FD_COLOR_FRIENDLY_ON = '#C7B6CE';
var FD_COLOR_FRIENDLY_OFF = '#5C6B85';
var FD_COLOR_COL_HEAD = '#A38FB0';
var FD_COLOR_DT_LABEL = '#9A86A8';
var FD_COLOR_DT_LOW = '#8a508f';
var FD_COLOR_DT_OK = '#66BB6A';
var FD_COLOR_DT_HIGH = '#FF6361';
...
function buildFlowDiagram() {
  var W = 1160, H = 460;
    ...
    p.push('<radialGradient id="fglow" cx="22%" cy="34%" r="72%"><stop offset="0%" stop-color="rgba(122,167,206,0.2)"/><stop offset="48%" stop-color="rgba(240,121,91,0.1)"/><stop offset="100%" stop-color="transparent"/></radialGradient>');
    p.push('<linearGradient id="lbgrad" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#9E4A18"/><stop offset="100%" stop-color="#ff8531"/></linearGradient>');
    for (var gz = 1; gz <= ZONES; gz++) {
      p.push('<linearGradient id="rg' + gz + '" x1="0" y1="0" x2="1" y2="0">');
      p.push('<stop id="rgs' + gz + '" offset="0%" stop-color="#ff8531"/>');
      p.push('<stop id="rga' + gz + '" offset="100%" stop-color="#8a508f"/>');
      p.push('</linearGradient>');
    }
    p.push('</defs>');
    p.push('<rect width="' + W + '" height="' + H + '" rx="22" fill="#042a3b"/>');
    ...
    for (var z = 1; z <= ZONES; z++) {
      ...
      p.push('<path d="' + bgD + '" fill="#062a3a" opacity="0.9"/>');
    }
    ...
    p.push('<line x1="' + FD_EX + '" y1="36" x2="' + FD_EX + '" y2="' + (H - 36) + '" stroke="#ff8531" stroke-width="3" opacity="0.55"/>');
    ...
    // Orange source box
    p.push('<rect x="' + FD_BX + '" y="' + by + '" width="' + FD_BW + '" height="' + FD_BH + '" rx="6" fill="#ff8531"/>');
    p.push('<text x="' + (FD_BX + FD_BW / 2) + '" y="' + (FD_MYC - 10) + '" text-anchor="middle" font-size="20" font-weight="800" fill="#00202e" letter-spacing="2">FLOW</text>');
    p.push('<text id="fd-flow-temp" x="' + (FD_BX + FD_BW / 2) + '" y="' + (FD_MYC + 22) + '" text-anchor="middle" font-size="26" font-weight="800" fill="#00202e" font-family="var(--mono)">---</text>');

    // Return temp under box
    p.push('<text id="fd-ret-temp" x="' + (FD_BX + FD_BW / 2) + '" y="' + (by + FD_BH + 28) + '" text-anchor="middle" font-size="17" font-weight="700" fill="#8a508f" font-family="var(--mono)">RET ---</text>');
    ...
    for (z = 1; z <= ZONES; z++) {
      var yy = FD_ZY[z - 1];
      p.push('<text id="fd-zn' + z + '" x="' + FD_COL_ZONE_X + '" y="' + (yy + 2) + '" font-size="14" fill="#FFEAD2" font-weight="700" letter-spacing="2">ZONE ' + z + '</text>');
      p.push('<text id="fd-zf' + z + '" x="' + FD_COL_ZONE_X + '" y="' + (yy + 18) + '" font-size="11" fill="#B6A6C0" font-weight="700" letter-spacing="1">---</text>');
      ...
      p.push('<text id="fd-zt' + z + '" x="' + FD_COL_TEMP_X + '" y="' + (yy + 10) + '" font-size="16" fill="#F6ECE0" font-weight="700" font-family="var(--mono)">---°C</text>');
      p.push('<text id="fd-zv' + z + '" x="' + FD_COL_FLOW_X + '" y="' + (yy + 10) + '" font-size="16" fill="#C7B7D0" font-weight="700" font-family="var(--mono)">---%</text>');
      p.push('<text id="fd-zr' + z + '" x="' + FD_COL_RET_X + '" y="' + (yy + 10) + '" font-size="16" fill="#C7B7D0" font-weight="700" font-family="var(--mono)">---</text>');
    }
    ...
    p.push('<text id="fd-dt" x="36" y="' + (H - 16) + '" font-size="36" font-weight="800" fill="#ff8531" font-family="var(--mono)">---</text>');

    p.push('<text x="' + (W - 36) + '" y="' + (H - 14) + '" text-anchor="end" font-size="15" fill="#0C3A52" font-weight="700" letter-spacing="6">UFH - ' + ZONES + ' ZONES - HEATVALVE</text>');
    p.push('</svg>');
